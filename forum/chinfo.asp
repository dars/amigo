...
%>
<%=t1%>錯 誤 訊 息<%=t2&d1%>·對不起！請填寫完整必填的項目· <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><%=d2%>
<%response.end
end if%>
<%if repwd<>newpwd then%>
<%=t1%>錯 誤 訊 息<%=t2&d1%>·你的重複密碼與你的新密碼不匹配· <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><%=d2%>
<%response.end
end if%>
<%
...
%><%=t1%>錯 誤 訊 息<%=t2&d1%>·請檢查你的 <b>E-mail</b> 是否填寫準確· <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><%=d2%>
<%response.end
end if%>
<%mytp=mypic
if mypic="" then
...
%><%=t1%>錯 誤 訊 息<%=t2&d1%>·你的圖像大小設定錯誤· <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><%=d2%>
<%response.end
end if%>
<%
...
%>
<%=t1%>修 改 成 功<%=t2&d1%>·你已經完成的修改了你的用戶信息！如果你有修改密碼，請 <a href="login.asp">重新登陸</a>·<%=d2%>
