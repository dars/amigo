...
if abc="no" then
%><br><br>
<%=t1%>錯 誤 訊 息<%=t2&d1%>·你不是該帖的作者，不能編輯該帖子·<%=d2%>
<%
response.end
end if
set edit=nothing
...
·<a href="javascript:grade()" title='格式：[showtograde=1]內容[/s]			表示只有等級在「1」以上才能瀏覽「內容」			「1」和「內容」可以自定，				使用該項請點擊！'>等級可見內容</a><br>·<a href="javascript:reply()" title='格式：[showtoreply]內容[/s]			表示只有回覆者才能瀏覽「內容」，		「內容」可以自定，				使用該項請點擊！'>回覆可見內容</a><br>·<a href="javascript:name()" title='格式：[showtoname=zym]內容[/s]			表示只有「zym」才能瀏覽「內容」			「zym」和「內容」可以自定，				使用該項請點擊！'>指定讀者內容</a><br></td>      <td width="76%" valign="top">
  ...
  &nbsp;<textarea onkeydown=presskey(); rows=10 name=body cols=79 Title='按 Ctrl+Enter 直接發送'><%=edit("body")%></textarea><table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse" bordercolor="#111111" height="30">
          <tr>
            <td>
            &nbsp;點擊圖標插入表情：</td>
            <td>
...
<br>
&nbsp;<input type=submit value=OK_好了！修改帖子 name=B1>&nbsp; <input type=reset value=NO_不行！我要重寫 name=B2> [按  Ctrl+Enter 直接發送]<br><br>
...
%><br><br><%=t1%>修 改 成 功<%=t2&d1%>·修改完成 <a href="show.asp?id=<%=reid%>&bd=<%=bd%>">
<font color="<%=c1%>">回到帖子頁面·</font></a><%=d2%><%
