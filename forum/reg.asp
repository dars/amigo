...
mes="<br>對不起！"&kbbs(name)&" 已被人註冊了！！！ <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><br><br>"
%><%else
if name="" or password="" or repassword="" or anhao="" or email="" then
mes="<br>對不起！你不能完成地註冊用戶！！！請填寫完整必填的項目 <a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><br><br>"
%><%
...
mes="<br>對不起！你不能完成地註冊用戶！！！請檢查你的E-mail是否出錯！！<a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><br><br>"
%><%
else
if repassword<>password then
mes="<br>你的重複密碼與原密碼不相同！！<a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><br><br>"
%><%
else
%>
<%
...
mes="<br>你的圖像大小設定錯誤！！<a href='javascript:history.go(-1)'><img border=0 src=pic/re.gif>返 回</a><br><br>"
%>
<%else%>
...
myconn.Execute(abc)
noyes="注 冊 成 功！"
mes="<br><form method=POST action=bbselse.asp name=login>恭喜你！ <b>"&kbbs(name)&"</b> 完成註冊！<input type=hidden name=lgname size=20 value="&name&"><input type=hidden name=lgpwd size=20 value="&password&"><a href='javascript:document.login.submit()'><img border=0 src=pic/go.gif align=absmiddle> 進入論壇</a></form>"
%>    
<%end if
end if
end if
